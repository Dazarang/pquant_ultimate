...
def load_tickers(json_path):
    ...
    return {"us_filtered": us_tickers, "sp500": sp500, "sweden": sweden}
...
def check_data_quality(ticker, df, current_date):
    ...
    if days_since_last > 90:
        return {"status": "delisted", "reason": f"stopped_trading_{days_since_last}_days_ago", "stats": stats}

    # Price collapsed (bankruptcy signal)
    if last_price < 1.0 and avg_price > 5.0:
        return {"status": "delisted", "reason": "price_collapsed_to_penny_stock", "stats": stats}
...
def download_batch(tickers, start_date, end_date, current_date):
    ...
    """
    results = {"valid": [], "delisted": [], "rejected": []}
        ...
        for ticker in tickers:
            try:
                ...
                if quality["status"] == "valid":
                    results["valid"].append({"ticker": ticker, "data": ticker_data, "stats": quality["stats"]})
...
def download_all_tickers(ticker_list, start_date="2015-01-01", end_date="2024-12-31"):
    ...
    batch_size = 100
    all_results = {"valid": [], "delisted": [], "rejected": []}
    ...
    if use_market_cap:
        ...
        if len(df_us) > 0:
            df_us["cap_bucket"] = pd.cut(
                df_us["market_cap"], bins=[0, 2e9, 10e9, 200e9, 1e15], labels=["small", "mid", "large", "mega"]
            )
        ...
        print("⚠️  Insufficient market cap data, using volume-based selection")

        # Volume buckets
        df["volume_bucket"] = pd.qcut(
            df["avg_volume"], q=4, labels=["low", "medium", "high", "very_high"], duplicates="drop"
        )
...
def main():
    ...
    try:
        git_commit = (
            subprocess.check_output(
                ["git", "rev-parse", "--short", "HEAD"], cwd=Path(__file__).parent.parent, stderr=subprocess.DEVNULL
            )
            .decode("utf-8")
            .strip()
        )
    ...
    print("  Data frequency: Daily (default yfinance interval)")
    ...
    # Step 5: Stratified selection
    selected_tickers, selection_stats = stratified_selection(enriched, download_results["delisted"], TARGET_FINAL)

    # Step 6: Prepare final dataset
    final_data = prepare_final_dataset(selected_tickers, enriched, download_results["delisted"])
    ...
    if not all_dfs:
        ...
        print("   Try adjusting the quality filters or ticker selection")
        return
