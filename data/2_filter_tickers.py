...
def filter_junk_tickers(tickers):
    ...
    filtered = [
        ...
        for t in tickers
        if not (
            "$" in t  # Preferred shares with $ (ABR$D, ACR$C)
            or t.endswith("W")  # Warrants
            or t.endswith("U")  # Units
            or t.endswith("R")  # Rights
            or ".W" in t  # Warrants (alt format)
            or ".U" in t  # Units (alt format)
            or "+" in t  # Special shares
            or "=" in t  # When-issued
            or "^" in t  # Preferred (some formats)
            or len(t) > 6  # Likely derivatives
            or t.endswith("WW")  # Double warrants
        )
    ]
    ...
    filtered = [
        ...
        for t in filtered
        if not any(
            t.endswith(suffix) for suffix in ["P", "PR", "PRA", "PRB", "PRC", "PRD", "PRE", "PRF", "PRG", "PRH", "PRI"]
        )
    ]
    ...
    filtered = [t for t in filtered if not any(t.startswith(pattern) for pattern in etf_patterns)]
...
def main():
    ...
    print(f"Removed: {original_total - filtered_total} ({(original_total - filtered_total) / original_total:.1%})")
    print("\nRemoved categories:")
    print("  - Preferred shares ($, P, PR, letter series)")
    print("  - Warrants, units, rights")
    print("  - ETFs")
    print("  - Duplicates (US tickers already in SP500)")
    print("  - Class share redundancy (kept B, removed A)")
    print(f"\nSaved: {OUTPUT_JSON}")
