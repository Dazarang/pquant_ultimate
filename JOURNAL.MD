...
- `data.py` - load_dataset(), temporal_split(), scale(), list_features(), preview()
- `eval.py` - 2-tier eval (ranking quality + multi-budget composite), select_top_frac, backtest_quick, benchmark_random_entry
...
### Research (`research/`)
Autoresearch system (Karpathy-style autonomous ML research):
- `experiment.py` - Mutable: model, hyperparams, features, stocks
- `features_lab.py` - Mutable: custom feature engineering
- `baseline.py` - Immutable: logistic regression reference
- `program.md` - Agent instructions and constraints
- `gate.sh` - Verification gate (immutability, timeout, sanity)
- `run.sh` - Outer loop (agent dispatch or manual mode)
- `RESEARCH_LOG.md` / `COMBAT_LOG.md` - Experiment tracking
...
### 2026-03-30
Replaced threshold-based composite score with threshold-free multi-budget evaluation. Previous metric was gameable (Goodhart's Law): researcher pushed threshold to 0.85, got 1 signal on val scoring 11.02 -- useless. New eval: model outputs probabilities only, judge selects signals at 5 budgets (0.10%-2%) x 3 horizons (5/10/20d), linear N-scaling via eff_n/(eff_n+50), missing cells excluded from average. THRESHOLD removed from experiment.py. All logs cleared for fresh research run.

Follow-up: fixed 3 correctness issues in scorer. P1: cutoff ties now use fractional weights (row-order invariant). P2: plot annotations use shared forward_open_return() helper, skip signals without full 10d window. P3: budget details keyed by raw float, not rounded label (no collision on custom budgets). Added test_and_plot.py for held-out test evaluation. Baseline reset to -2.4916. 56 eval tests, 177 total pass.

### 2026-03-28
Overhauled 3-tier eval (lib/eval.py): fixed composite score dimensional bug (win_rate now centered on 0.5 and properly scaled), added MAE path-risk penalty, uses excess return vs equal-weight market benchmark instead of raw return, multi-horizon Tier 2 gate (any horizon positive excess, not just mean_10d > 0), signal clustering metric (effective N), regime breakdown (bull/bear). Breaking change: historical scores incomparable.

### 2026-03-28
Changed pivot labeling to use window_variations=[-1, 1] with price_tolerance=0.01 (1%). Labels days adjacent to exact bottoms if close is within 1%. Increases positive labels ~58% (1:30 to 1:19 imbalance). Forward return quality barely changes (7.46% to 6.81% mean 10d). Dataset rebuilt.

### 2026-03-28
Built autoresearch system (research/). Karpathy-style autonomous loop: agent edits experiment.py + features_lab.py, gate.sh verifies immutability + runs + extracts composite score, run.sh orchestrates keep/revert/log. Baseline score: -0.25 (logistic regression). Starting experiment score: 2.21 (GradientBoosting, 5 stocks, base features). Added xgboost, lightgbm deps.

### 2026-03-28
Phase 2 finalized. Added: embargo gap (13 sessions) in temporal_split, stock/feature filtering in load_dataset, FEATURES catalog in features.py, list_features() and preview() in data.py, 3-tier eval (classification + forward returns + composite score) with tiered_eval() funnel. Tests: test_data.py (29) + test_eval.py (23) = 52 passing.

### 2026-03-27
Phase 2 started. Added lib/data.py (load_dataset, temporal_split, scale) and lib/eval.py. PivotLow only -- PivotHigh dropped on load. Added scikit-learn dependency.
